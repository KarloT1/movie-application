...
const MovieDetails = () => {
  ...
  const getMovieDetails = async () => {
    await moviesAPI.getMovieDetails(params.movieId).then(res => {
      if (res.ok) {
        return res.json()
      } else {
        setError(true)
  ...
  const getSimilarMovies = async () => {
    await moviesAPI.getSimilarMovies(params.movieId).then(res => {
      if (res.ok) {
        return res.json()
      } else {
        setError(true)
    ...
    }).then(similarMovies => setSimilar(similarMovies.results))
      .then(() => setLoading(false))
  }
