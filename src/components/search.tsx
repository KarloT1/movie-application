...
const Search = () => {
  ...
  useEffect(() => {
    const searchBooks = () => {
      query.length && (
        moviesAPI.searchMovies(query).then(res => {
          if (res.ok) {
            return res.json()
          } else {
            setError(true)
  ...
  // On Enter press navigate user to search results and empty the query
  ...
  return (
    <>
      {error
      ? <h5 className="text-light">Search isn't working right now. Try again later.</h5>
      : <div className="input-group w-50 position-relative">
          ...
          {/* Only show search dropdown if num of entered letters is greate than 2 */}
